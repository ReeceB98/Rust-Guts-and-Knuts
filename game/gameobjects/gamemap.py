...
class GameMap:
    ...
    def __init__(self, renderer: pyasge.Renderer, tmx_file: str, data: GameData) -> None:
        ...
        data.off_limits = []
        for rect in tmxdata.layernames["OffLimits"]:
            data.off_limits.append([rect.x, rect.y, rect.width, rect.height])

        data.robot_spawns = []
        for obj in tmxdata.layernames["robotSpawns"]:
            data.robot_spawns.append([obj.x, obj.y])

        data.bottle_spawns = []
        for obj in tmxdata.layernames["bottleSpawns"]:
            data.bottle_spawns.append([obj.x, obj.y])
